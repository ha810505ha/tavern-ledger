...
Track the model, tokens and cost of each generation, browse spending across months, and export detailed records. Ledger data is stored in SillyTavern's per-user files.

## Screenshots

These are real interface screenshots in Traditional Chinese. Amounts and records vary by ledger.

**Extension settings**

![Tavern Ledger extension settings](docs/images/settings-panel.jpg)

**Ledger overview and individual records**

![Tavern Ledger overview and individual records](docs/images/ledger-overview.png)

## Requirements

- SillyTavern **Chat Completion → OpenRouter** with a standard OpenRouter API key; a management key is not required.
- Cost tracking is confirmed on SillyTavern 1.17.0, but account balance lookup is unavailable there. See [Compatibility and scope](#compatibility-and-scope) for the confirmed test results.
...
- Unconfirmed costs remain in individual records without a persistent summary notice. Each request gets at most three account-difference checks; reopening the ledger does not repeat them.
...
| SillyTavern 1.17.0 | Cost tracking works; account balance lookup is unavailable |
